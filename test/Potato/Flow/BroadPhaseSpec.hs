  ...
  describe "BroadPhase" $ do
    ...
    it "passes basic test" $ do
      let
        ...
        (aabbs1, BroadPhaseState bpt1) = update_bPTree changes1 bpt0
      ...
      let
        ...
        (aabbs2, BroadPhaseState bpt2) = update_bPTree changes2 bpt1
        culled2 = broadPhase_cull lb1_1 bpt2
      length aabbs2 `shouldBe` 3
      length culled2 `shouldBe` 4
      let
        changes3 = makeChange 1 lb1_2
        (aabbs3, BroadPhaseState bpt3) = update_bPTree changes3 bpt2
    ...
    it "click on corner works as expected" $ do
      let
        ...
        (aabbs1, BroadPhaseState bpt1) = update_bPTree changes bpt0
